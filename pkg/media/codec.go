...
// standardSampleRates are the common audio clock rates a measured rate is
// snapped to. They are spaced far enough apart (each ~2x the previous) that a
// measurement within ±25% still rounds to the correct rate, absorbing jitter.
var standardSampleRates = []int{8000, 16000, 32000, 48000}

// nearestStandardRate rounds an empirically measured sample rate (e.g. decoded
// samples or RTP timestamp units per wall-clock second) to the closest common
// codec clock rate. Returns 0 for a non-positive input.
func nearestStandardRate(measured float64) int {
	if measured <= 0 {
		return 0
	}
	best := standardSampleRates[0]
	bestDiff := math.Abs(measured - float64(best))
	for _, r := range standardSampleRates[1:] {
		if d := math.Abs(measured - float64(r)); d < bestDiff {
			bestDiff = d
			best = r
		}
	}
...
// OutputSampleRate returns the sample rate of the 16-bit PCM that the decoder
// emits for the given codec. This can differ from the RTP clock rate carried in
// the SDP rtpmap: G.722's rtpmap clock rate is advertised as 8000 Hz per
// RFC 3551 §4.5.2, but it is a wideband codec whose decoder produces 16 kHz PCM.
// Recording (WAV) headers must use this decoded rate, not the RTP clock rate, or
// playback runs at half speed. rtpClockRate is used as the fallback for codecs
// whose decoded rate equals their advertised clock rate.
func OutputSampleRate(codecName string, rtpClockRate int) int {
