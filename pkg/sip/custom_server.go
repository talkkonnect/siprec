	...
	if recordingSession != nil {
		...
		if receivedSDP != nil {
			...
			if len(audioStreams) == 1 {
				logger.WithFields(logrus.Fields{
					"audio_stream_count": len(audioStreams),
					"call_id":            message.CallID,
				}).Warn("SIPREC offer contains only one audio stream: only a single participant leg will be recorded. " +
					"A two-party call should present two m=audio streams (producing leg0 and leg1). " +
					"If the recorded leg is silent, verify the SRC (e.g. FreeSWITCH mod_siprec) is exporting both participant media streams and that media is bridged.")
			}
		} else {
		...
		if parsedSDP != nil {
			...
			if len(audioStreams) == 1 {
				logger.WithFields(logrus.Fields{
					"audio_stream_count": len(audioStreams),
					"call_id":            message.CallID,
				}).Warn("SIPREC re-INVITE contains only one audio stream: only a single participant leg will be recorded. " +
					"A two-party call should present two m=audio streams (producing leg0 and leg1). " +
					"If the recorded leg is silent, verify the SRC (e.g. FreeSWITCH mod_siprec) is exporting both participant media streams and that media is bridged.")
			}
		}
